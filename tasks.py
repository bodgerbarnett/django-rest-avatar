...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-rest-avatar runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-rest-avatar.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ rest_avatar")
...
def lint(c):
    ...
    """
    c.run("flake8 django-rest-avatar tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=rest_avatar.__version__))
    c.run("git push --tags")
